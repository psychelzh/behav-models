...
parameters {
  ...
  cholesky_factor_corr[2] L_R_mu_base;
  cholesky_factor_corr[2] L_R_sigma_base;
  cholesky_factor_corr[2] L_R_mu_delta;
  cholesky_factor_corr[2] L_R_sigma_delta;
...
transformed parameters {
  ...
  mu_i_base_tilde = diag_pre_multiply(mu_sd_base, L_R_mu_base) * mu_i_base_pr;
  sigma_i_base_tilde = diag_pre_multiply(sigma_sd_base, L_R_sigma_base) * sigma_i_base_pr;
  mu_i_delta_tilde = diag_pre_multiply(mu_sd_delta, L_R_mu_delta) * mu_i_delta_pr;
  sigma_i_delta_tilde = diag_pre_multiply(sigma_sd_delta, L_R_sigma_delta) * sigma_i_delta_pr;

  // Compute individual-level parameters from non-centered parameterization
  for (i in 1:N) {
    // Congruent at time 1
    mu_i_base[i,1] = mu_mean_base[1] + mu_i_base_tilde[1,i];
    // Congruent at time 2
    mu_i_base[i,2] = mu_mean_base[2] + mu_i_base_tilde[2,i];

    // Congruent at time 1
    sigma_i_base[i,1] = sigma_mean_base[1] + sigma_i_base_tilde[1,i];
    // Congruent at time 2
    sigma_i_base[i,2] = sigma_mean_base[2] + sigma_i_base_tilde[2,i];
...
model {
  ...
  L_R_mu_base    ~ lkj_corr_cholesky(1);
  L_R_sigma_base ~ lkj_corr_cholesky(1);
  L_R_mu_delta    ~ lkj_corr_cholesky(1);
  L_R_sigma_delta ~ lkj_corr_cholesky(1);
...
generated quantities {
  ...
  corr_matrix[2] R_mu_base;
  corr_matrix[2] R_sigma_base;
  corr_matrix[2] R_mu_delta;
  corr_matrix[2] R_sigma_delta;
  ...
  R_mu_base = L_R_mu_base * L_R_mu_base';
  R_sigma_base = L_R_sigma_base * L_R_sigma_base';
  R_mu_delta = L_R_mu_delta * L_R_mu_delta';
  R_sigma_delta = L_R_sigma_delta * L_R_sigma_delta';
