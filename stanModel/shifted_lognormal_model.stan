...
parameters {
  ...
  matrix[2,N] mu_i_delta_pr; // transposed for faster computation
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    // Phi_approx  fast approximation of the unit normal cumulative distribution, value between 0 and 1
    ndt_i[i,1] = Phi_approx(ndt_mean[1] + ndt_sd[1] * ndt_i_pr[i,1]) * min(RT_min[i,,1]);  // min(RT_min[i,,1]) is the minimum RT across conditions at time 1
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:T_subj[i,1,1]) {
      ...
      target += shiftlnorm_lpdf(RT[i,1,1,t] | ndt_i[i,1], mu_i_base[i,1], exp(sigma_i_base[i,1]));
    }
    ...
    //                                 exp(sigma_i_base[i,1] + sigma_i_delta[i,1]));
      ...
      target += shiftlnorm_lpdf(RT[i,2,1,t] | ndt_i[i,1], mu_i_base[i,1] + mu_i_delta[i,1], exp(sigma_i_base[i,1] + sigma_i_delta[i,1]));
    }
    // Congruent at time 2
    ...
    // }
    target += shiftlnorm_lpdf(RT[i,1,2,t] | ndt_i[i,2], mu_i_base[i,2], exp(sigma_i_base[i,2]));
    }
    ...
    // }
    target += shiftlnorm_lpdf(RT[i,2,2,t] | ndt_i[i,2], mu_i_base[i,2] + mu_i_delta[i,2], exp(sigma_i_base[i,2] + sigma_i_delta[i,2]));
    }
  }
}
